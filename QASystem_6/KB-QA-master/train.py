...
import sys
import numpy as np
os.environ["CUDA_VISIBLE_DEVICES"]="0"#改成0就是0号GPU
...
tf.flags.DEFINE_string("result_file", "results/predictRst.score", "Predict result.")
tf.flags.DEFINE_string("save_file", "results/savedModel", "Save model.")
...
# hyperparameters
tf.flags.DEFINE_integer("k", 5, "K most similarity knowledge (default: 5).")#选前五个最相近的知识
...
# training parameters
tf.flags.DEFINE_integer("batch_size", 128, "Batch Size (default: 64)")#原来256
...
embedding, word2idx = data_util.load_embedding(FLAGS.embedding_file)#导入已有的embeddingfile（使用的是2017维基百科的word2vec）
...
print("loading data...")#将最相似的几个格式化一下归入训练集（是问题或是答案 正确或者错误）
...
with tf.Graph().as_default(), tf.device(FLAGS.gpu_device):
    ...
    with tf.Session(config=session_conf).as_default() as sess:
        ...
        out_dir = os.path.abspath(os.path.join(os.path.curdir, "output", timestamp))#实时保存##########################
        print("Writing to {}\n".format(out_dir))
